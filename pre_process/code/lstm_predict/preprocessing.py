...
def percentile_remove_outlier(df, filter_start, filter_end):
    ...
    outlier_low = q1 - (q3 - q1) * 6
    outlier_high = q3 + (q3 - q1) * 6
    print("outlier_low,outlier_high",outlier_low,outlier_high)
    # make sure every element in the filtering region within normal range
    print([((filter_region > outlier_low) & (filter_region < outlier_high)).sum(axis=1) == (filter_end - filter_start) + 1])
    df_fil = df.ix[
...
def MinMaxScaler(df):
    ...
    maxValue = np.asarray(df).ravel().max()
    minValue = np.asarray(df).ravel().min()
    df = (df - minValue) / (maxValue - minValue)
    bin_boundary = [(x - minValue) / (maxValue - minValue) for x in [0,50,75,90]]
    return df, minValue, maxValue, bin_boundary
...
def NormalDistributionScaler(df, start_col_index, end_col_index):
    ...
    df.ix[:, start_col_index:end_col_index] = df.ix[:, start_col_index:end_col_index].apply(
        lambda x: (x - mean) / std)
    bin_boundary = [(x-mean) / std for x in [0,50,75,90]]
    return df, mean, std, bin_boundary


def get_ids_and_files_in_dir(inputdir, range, input_file_regx="(DBID)\((\d+)\)_INSTID\([1]\).csv"):            # "^(\d+)\.csv"
    """
    ...
    for file in os.listdir(inputdir):
        ...
        if pattern_match:
            current_id = pattern_match.group(2)
...
def binning_date_y(df, y_col=-1, n_group=5, bin_boundary = [1,2,3]):
    """
