...
if  __name__ == '__main__':
    ...
    if not os.path.exists(pkl_path):
        ...
        df_load_regr, df_load_cla, df_perf_regr, df_perf_cla = pkl.load(output)
        output.close()
        
    perfect_percentage = 0.5
    
    df_load_regr_un = df_load_regr[df_load_regr["feature_select"].isin(["lasso", "pearson", "reliefF"]) & df_load_regr["feature_percentage"].isin([perfect_percentage,])]
    df_load_cla_un = df_load_cla[df_load_cla["feature_select"].isin(["lasso", "pearson", "reliefF"]) & df_load_cla["feature_percentage"].isin([perfect_percentage,])]
    df_perf_regr_un = df_perf_regr[df_perf_regr["feature_select"].isin(["lasso", "pearson", "reliefF"]) & df_perf_regr["feature_percentage"].isin([perfect_percentage,])]
    df_perf_cla_un = df_perf_cla[df_perf_cla["feature_select"].isin(["lasso", "pearson", "reliefF"]) & df_perf_cla["feature_percentage"].isin([perfect_percentage,])]
